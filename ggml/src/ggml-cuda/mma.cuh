...
static __device__ __forceinline__ int ggml_cuda_movmatrix(const int x) {
    ...
    const int ret_low  = (__shfl_sync(0xffffffffffffffffULL, x, src_laneid_low,  WARP_SIZE) >> shift_low)  & 0x0000FFFF;
    const int ret_high = (__shfl_sync(0xffffffffffffffffULL, x, src_laneid_high, WARP_SIZE) << shift_high) & 0xFFFF0000;
